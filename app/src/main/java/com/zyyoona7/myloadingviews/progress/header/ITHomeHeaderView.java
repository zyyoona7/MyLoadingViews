...
public class ITHomeHeaderView extends FrameLayout implements IHeaderView {
    ...
    private ITHomeLoadingView mLoadingView;

    public ITHomeHeaderView(@NonNull Context context) {
        this(context, null);
    }

    public ITHomeHeaderView(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    private void init(Context context) {
        LayoutInflater.from(context).inflate(R.layout.header_it_home, this);
        mLoadingView = (ITHomeLoadingView) findViewById(R.id.itlv_header);
        mLoadingView.setStrokeWidth(1.5f);
        mLoadingView.setRingSpacing(2f);
        mLoadingView.setFontSize(12f);
    }
